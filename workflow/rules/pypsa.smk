...
rule clean:
    run:
        shell(f"snakemake -j 1 solve_all_networks --delete-all-output --directory {workflow.workdir}")
        try:
            shell(f"snakemake -j 1 solve_all_networks_monte --delete-all-output --directory {workflow.workdir}")
        except:
            pass
        shell(f"snakemake -j 1 run_all_scenarios --delete-all-output --directory {workflow.workdir}")
...
rule run_tests:
    ...
    run:
        ...
        shell(f"snakemake --cores all build_test_configs --directory {workflow.workdir}")
        ...
        for filename in os.scandir(directory):  # iterate over files in that directory
            if filename.is_file():
                ...
                if "custom" in filename.name:
                    ...
                    shell(f"snakemake --cores 1 run_all_scenarios --forceall --directory {workflow.workdir}")
                if "monte" in filename.name:
                    shell("cp {filename.path} config.yaml")
                    shell(f"snakemake --cores all solve_all_networks_monte --forceall --directory {workflow.workdir}")
                else:
                    shell("cp {filename.path} config.yaml")
                    shell(f"snakemake --cores all solve_all_networks --forceall --directory {workflow.workdir}")
...
rule run_scenario:
    ...
    run:
        ...
        print("IMA HERE")
        os.system(f"snakemake -j all solve_all_networks --rerun-incomplete")
        os.system(f"snakemake -j1 make_statistics --force")
